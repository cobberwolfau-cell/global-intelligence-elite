...
export default function Home() {
  ...
  const loadAnalysis = useCallback(async () => {
    ...
    try {
      ...
      let newsContext = '';
      try {
        const newsCategory = appMode === 'finance' ? financeCategory : intelCategory;
        const newsPageSize = Math.min(itemCount, 10);
        const newsResp = await fetch(
          `/api/trpc/news.fetch?input=${encodeURIComponent(JSON.stringify({ json: { country: selectedCountry, category: newsCategory, pageSize: newsPageSize } }))}`,
          { credentials: 'include' }
        );
        if (newsResp.ok) {
          const newsJson = await newsResp.json() as { result?: { data?: { json?: { articles?: Array<{ source: string; title: string; url: string }> } } } };
          const articles = newsJson?.result?.data?.json?.articles;
          if (articles && articles.length > 0) {
            const articleLines = articles
              .slice(0, itemCount)
              .map((a: { source: string; title: string; url: string }, i: number) => `${i + 1}. [${a.source}] ${a.title}${a.url ? ' — ' + a.url : ''}`)
              .join('\n');
            newsContext = `\n\n【真實新聞參考資料（請優先以下列真實新聞為基礎進行分析，並使用其真實連結）】：\n${articleLines}\n請將上述真實新聞的標題、出處媒體和連結直接用於【報導標題】、【出處】和【連結】欄位。`;
          }
        }
      } catch {
        // NewsAPI unavailable, continue without real news
      }
      ...
      const prompt = appMode === 'intel'
        ? isTravelCategory
          ? `${dateContext}${depthNote}你是一名資深旅遊情報分析師。請以繁體中文針對「${currentCountryLabel}」整理屬於「${currentTimeframeLabel}」視角的 ${itemCount} 則旅遊資訊。【刊報日期】必須填寫 ${todayStr} 或近期實際日期，不得使用過舊日期。\n【消息來源優先順序】：①優先引用當地主流新聞媒體及旅遊媒體（如當地報紙、旅遊雜誌、旅遊網站）；②其次引用國際旅遊網站或可信賴的旅遊資訊平台；③最後才引用當地旅遊局或政府官方旅遊網站。每則旅遊資訊的【出處】必須按此優先順序選擇最適合的來源。\n嚴格格式：\n1. 標題\n【刊報日期】：${todayStr}\n【可信度】：1-100\n【主體】：約 ${wordSpec.main} 字，涵蓋景點、交通、住宿、美食、節慶或旅遊安全\n【旅遊建議】：約 ${wordSpec.analysis} 字，實用的行程或注意事項\n【影響】：約 ${wordSpec.impact} 字，對旅遊計畫的影響\n【出處】：媒體、旅遊局或研判來源（機構全名）——優先選當地主流旅遊媒體，其次國際旅遊平台，最後政府旅遊局\n【報導標題】：該報導或文章的具體標題，若為研判則填「研判」\n【連結】：請盡量填寫該媒體或機構的真實官方網址（如 https://www.reuters.com 或 https://www.cna.com.tw），若確實無法提供則填 N/A\n最後加上【整體旅遊評估】：內容，包含安全等級與推薦季節。${newsContext}`
          : `${dateContext}${depthNote}你是一名資深地緣戰略情報官。請以繁體中文針對「${currentCountryLabel}」的「${currentIntelCategoryLabel}」整理屬於「${currentTimeframeLabel}」視角的 ${itemCount} 則情報。【刊報日期】必須填寫 ${todayStr} 或近期實際日期，不得使用過舊日期。\n【消息來源優先順序】：①優先引用當地主流新聞媒體（如當地報紙、電視台、通訊社）；②其次引用國際新聞網站或可信賴的網頁資訊；③最後才引用當地政府官方網站或政府新聞稿。每則情報的【出處】必須按此優先順序選擇最適合的來源。\n嚴格格式：\n1. 標題\n【刊報日期】：${todayStr}\n【可信度】：1-100\n【主體】：約 ${wordSpec.main} 字\n【分析】：約 ${wordSpec.analysis} 字\n【影響】：約 ${wordSpec.impact} 字\n【出處】：媒體、機構或研判來源（機構全名）——優先選當地主流媒體，其次國際媒體，最後政府官網\n【報導標題】：該報導或文章的具體標題，若為研判則填「研判」\n【連結】：請盡量填寫該媒體或機構的真實官方網址（如 https://www.reuters.com 或 https://www.cna.com.tw），若確實無法提供則填 N/A\n最後加上【總體戰略研判】：內容。${newsContext}`
        : `${dateContext}${depthNote}你是一名宏觀市場策略師。請以繁體中文針對「${currentFinanceCategoryLabel}」整理屬於「${currentTimeframeLabel}」視角的 ${itemCount} 則市場動態。【刊報日期】必須填寫 ${todayStr} 或近期實際日期，不得使用過舊日期。\n【消息來源優先順序】：①優先引用當地主流財經媒體（如當地財經報紙、財經電視台、本地交易所公告）；②其次引用國際財經媒體（如 Bloomberg、Reuters、Financial Times）；③最後才引用政府央行或金融監管機構官方網站。每則市場動態的【出處】必須按此優先順序選擇最適合的來源。\n嚴格格式：\n1. 標題\n【最新報價】：當前估算數字與漲跌幅\n【資產標的】：具體標的名稱\n【趨勢判定】：看多 / 看空 / 震盪\n【市場分析】：約 ${wordSpec.market} 字\n【關鍵點位】：一句話\n【期貨動向】：約 ${wordSpec.futures} 字\n【出處】：媒體、機構或研判來源（機構全名）——優先選當地財經媒體，其次國際財經媒體，最後政府央行官網\n【報導標題】：該報導或文章的具體標題，若為研判則填「研判」\n【連結】：請盡量填寫該媒體或機構的真實官方網址（如 https://www.bloomberg.com 或 https://www.reuters.com），若確實無法提供則填 N/A\n最後加上【總體資金流向研判】：內容。${newsContext}`;
      const text = await callDeepSeek('', [
        { role: 'system', content: `你是專業終端機引擎，必須嚴格依照格式輸出，不要使用 Markdown 表格。今天日期是 ${todayStr}，所有【刊報日期】必須使用 ${todayStr} 或近期實際日期，不得使用過舊日期。【出處選擇原則】：優先引用當地主流新聞媒體，其次引用其他可信賴網頁資訊，最後才引用政府官方網站。` },
